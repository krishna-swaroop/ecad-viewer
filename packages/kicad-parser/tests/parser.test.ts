    ...
    describe.each(schematicFiles)("%s", (filePath) => {
        ...
        function normalizeForSort(value: unknown): unknown {
            if (typeof value === "number") {
                ...
                if (
                    processed["data"] &&
                    typeof processed["data"] === "string" &&
                    processed.hasOwnProperty("ppi")
                ) {
        ...
        function semanticComparator(
            ...
            if (typeof obj1 === "number" && typeof obj2 === "number") {
                return (
                    parseFloat(obj1.toFixed(6)) === parseFloat(obj2.toFixed(6))
                );
            }
            ...
            if (Array.isArray(obj1) && Array.isArray(obj2)) {
                ...
                if (isPointsArray1 || isPointsArray2) {
                    ...
                    for (let i = 0; i < obj1.length; i++) {
                        if (
                            !_.isEqualWith(obj1[i], obj2[i], semanticComparator)
                        )
                            return false;
                    }
                    return true;
            ...
            if (
                ...
                if (
                    processed1["data"] &&
                    typeof processed1["data"] === "string" &&
                    processed1.hasOwnProperty("ppi")
                ) {
                ...
                if (
                    processed2["data"] &&
                    typeof processed2["data"] === "string" &&
                    processed2.hasOwnProperty("ppi")
                ) {
                ...
                for (const key of allKeys) {
                    ...
                    if (!_.isEqualWith(val1, val2, semanticComparator))
                        return false;
                }
        ...
        it(`[${relPath}] parse → save → parse is semantically equivalent (round-trip 1)`, () => {
            ...
            expect(_.isEqualWith(parsed1, parsed2, semanticComparator)).toBe(
                true,
            );
        });
        ...
        it(`[${relPath}] parse → save → parse → save is stable (round-trip 2)`, () => {
            ...
            expect(_.isEqualWith(parsed1, parsed3, semanticComparator)).toBe(
                true,
            );
        });
        ...
        it(`[${relPath}] passes KiCad CLI ERC after serialization`, () => {
            ...
            try {
                ...
                expect(
                    _.isEqualWith(reparsed, rereparsed, semanticComparator),
                ).toBe(true);
